...
// ******************************************************************* Additional lines *************************************************
EyeState lastEyeState = E_EXPLORE;

int eyeWidthCurrent = 36;
int eyeHeightCurrent = 36;

unsigned long lastEyeUpdate = 0;
const unsigned long EYE_UPDATE_INTERVAL = 40;

#define EYE_Y_FIXED 0   // LOCK vertical movement

void enterEyeState(EyeState newState) {
...
void updateEyes() {
  ...
  if (eyeState == E_CAUTIOUS) {
    targetW = 24;
    targetH = 24;
  }
  else if (eyeState == E_ANGRY) {
    targetW = 42;
    targetH = 20;
  }

  if (eyeWidthCurrent < targetW) eyeWidthCurrent++;
  if (eyeWidthCurrent > targetW) eyeWidthCurrent--;

  if (eyeHeightCurrent < targetH) eyeHeightCurrent++;
  if (eyeHeightCurrent > targetH) eyeHeightCurrent--;

  roboEyes.setWidth(eyeWidthCurrent, eyeWidthCurrent);
  roboEyes.setHeight(eyeHeightCurrent, eyeHeightCurrent);

  roboEyes.setPosition(DEFAULT);  // 🔒 NO vertical movement
  roboEyes.update();
}
